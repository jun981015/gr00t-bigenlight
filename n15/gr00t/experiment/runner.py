...
class TrainRunner:
    def __init__(
        ...
        # TrainingArguments normalizes a string to a list in __post_init__.
        if report_to == "wandb" or "wandb" in (report_to or []):
...
class CriticTrainRunner(TrainRunner):
    ...
    def create_trainer(
        ...
        if hasattr(model.critic_head, "target_backbone_encoder"):
            trainer.add_callback(PolyakUpdateCallback(
                target_model=model.critic_head.target_backbone_encoder,
                source_model=model.critic_head.backbone_encoder,
                tau=model.critic_head.config.rl_config["tau"],
            ))
