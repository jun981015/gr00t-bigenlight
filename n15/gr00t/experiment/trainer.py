...
class DualBrainTrainer(transformers.Trainer):
    ...
    def _load_rng_state(self, checkpoint):
        # transformers 4.51 uses weights_only=True but only adds NumPy RNG globals
        # automatically for torch >= 2.6. This repo pins torch 2.5.1 / NumPy < 2.
        # Keep restricted unpickling; allow only the exact NumPy RNG array types.
...
class DualBrainRLTrainer(DualBrainTrainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        outputs = model(inputs)
        loss = outputs["loss"]
        if (self.state.global_step + 1) % max(1, self.state.logging_steps) == 0:
