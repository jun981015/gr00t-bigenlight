...
class GR00T_N1_5_DEAS_Critic(PreTrainedModel):
    ...
    def __init__(
        ...
        algorithm = critic_cfg.rl_config.get("algorithm", "deas")
        if algorithm == "iql":
            from .action_head.iql_critic import IQLCritic
            self.critic_head = IQLCritic(critic_cfg)
            self.config.critic_cfg = critic_cfg.to_dict()
        elif algorithm == "deas":
            self.critic_head = DEASCritic(critic_cfg)
        else:
            raise ValueError(f"Unknown critic algorithm: {algorithm}")
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if not from_gr00t_n1_5:
            ...
            critic_head_cfg = DEASCriticConfig(**pretrained_gr00t_n1_5_cfg["action_head_cfg"])
            critic_head_cfg.critic_config = critic_cfg
            critic_head_cfg.value_config = value_cfg
            critic_head_cfg.rl_config = rl_cfg
            new_cfg.critic_cfg = critic_head_cfg.to_dict()
