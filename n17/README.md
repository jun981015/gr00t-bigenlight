...
Local extension: [Offline RL with GR00T datasets](getting_started/offline_rl.md) provides
episode-safe RL transitions, a BC baseline, and a PyTorch SVF learner with a frozen-VLM
GR00T action-head adapter. This is a separate path from the supervised fine-tuning launcher.

Local dataset recipe: [Carrot in pot / UR7e](examples/carrot_in_pot/README.md) includes
non-destructive LeRobot v3 conversion, train/validation splits, data checks, and a two-GPU BC launcher.
