...
def check_resume_compatibility(training: TrainingConfig) -> None:
    ...
    """
    if training.max_run_seconds is not None and training.max_run_seconds <= 0:
        raise ValueError("max_run_seconds must be positive")
    if training.keep_latest_training_state and training.save_total_limit == 1:
        raise ValueError("Use save_total_limit=0 or >=2 to preserve the last committed checkpoint")
    if training.save_only_model and (
        training.keep_latest_training_state
        or training.max_run_seconds
        or training.first_save_step
        or training.save_interval_seconds
    ):
        raise ValueError("Full checkpoint policy requires save_only_model=False")
    if training.save_only_model and (
        training.resume_from_checkpoint or training.resume_checkpoint_path
    ):
        raise ValueError(
