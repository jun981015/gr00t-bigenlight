...
class FinetuneConfig:
    ...
    """FP32 disables autocast, TF32 and FlashAttention2; the default is unchanged."""
    ...
    """Optimizer steps between training metric logs (including W&B)."""
    ...
    """Resume an explicitly verified checkpoint instead of guessing the newest directory."""

    keep_latest_training_state: bool = False
    """After a full save commits, retain only the newest optimizer/scheduler/RNG state."""

    max_run_seconds: float | None = None
    """Save full state and stop after this training-time budget; leave margin for saving."""

    first_save_step: int | None = None
    """Optional early recovery checkpoint before the regular save cadence."""

    save_interval_seconds: float | None = None
    """Optional wall-clock recovery saves in addition to save_steps."""

    save_at_steps: list[int] = field(default_factory=list)
    """Additional explicit checkpoint steps, without restarting the optimizer/scheduler."""
    ...
    def __post_init__(self) -> None:
        ...
        if any(step <= 0 for step in self.save_at_steps):
            raise ValueError("save_at_steps must contain positive steps")
        for name in ("first_save_step", "save_interval_seconds"):
