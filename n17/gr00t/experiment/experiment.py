...
from gr00t.experiment.checkpoint_policy import ResumableCheckpointCallback, atomic_json
...
def run(config: Config):
    ...
    if (
        config.training.keep_latest_training_state
        or config.training.max_run_seconds is not None
        or config.training.first_save_step is not None
        or config.training.save_interval_seconds is not None
        or config.training.save_at_steps
    ):
        trainer.add_callback(
            ResumableCheckpointCallback(
                keep_latest=config.training.keep_latest_training_state,
                max_run_seconds=config.training.max_run_seconds,
                first_save_step=config.training.first_save_step,
                save_interval_seconds=config.training.save_interval_seconds,
                save_at_steps=config.training.save_at_steps,
            )
        )
    ...
    if config.training.enable_profiling:
        ...
        with torch.profiler.profile(
            ...
            trainer.train(
                resume_from_checkpoint=config.training.resume_checkpoint_path
                or config.training.resume_from_checkpoint
            )
    else:
        trainer.train(
            resume_from_checkpoint=config.training.resume_checkpoint_path
