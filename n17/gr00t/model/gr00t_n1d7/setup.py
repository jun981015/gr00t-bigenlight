...
class Gr00tN1d7Pipeline(ModelPipeline):
    ...
    def _create_model(self):
        ...
        nested_loading = dict(self.transformers_loading_kwargs)
        outer_loading = dict(self.transformers_loading_kwargs)
        precision_overrides = {}
        if self.model_config.model_dtype == "float32" and not self.model_config.use_flash_attention:
            # SDPA belongs to Qwen, not the custom GR00T wrapper, which does not
            # advertise HF's SDPA interface. Keep tokenizer loading kwargs clean.
            nested_loading.update(torch_dtype=torch.float32, attn_implementation="sdpa")
            outer_loading["torch_dtype"] = torch.float32
            precision_overrides = {"use_flash_attention": False, "model_dtype": "float32"}
        if self.config.training.start_from_checkpoint is not None and not skip_weight_loading:
            model, loading_info = AutoModel.from_pretrained(
                ...
                transformers_loading_kwargs=nested_loading,
