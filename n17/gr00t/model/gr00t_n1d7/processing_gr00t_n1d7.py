...
class Gr00tN1d7Processor(BaseProcessor):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str | Path, **kwargs):
        ...
        if kwargs:
            ...
            # The fine-tuning pipeline passes these settings explicitly. Ignoring
            # them silently retains a checkpoint's old crop/normalization recipe.
            clear_legacy_image_sizes = (
                kwargs.get("shortest_image_edge") is not None
                and kwargs.get("crop_fraction") is not None
            )
            override_keys = [
                "use_percentiles",
                "clip_outliers",
            ...
            for key in override_keys:
                if key in kwargs:
                    ...
                    if override is not None or (
                        clear_legacy_image_sizes and key in ("image_crop_size", "image_target_size")
                    ):
