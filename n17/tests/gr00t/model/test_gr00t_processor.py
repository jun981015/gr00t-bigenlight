...
def test_from_pretrained_honors_finetune_image_and_normalization_overrides():
    from gr00t.model.gr00t_n1d7.processing_gr00t_n1d7 import Gr00tN1d7Processor

    with patch(
        "gr00t.model.gr00t_n1d7.processing_gr00t_n1d7.build_processor", return_value=MagicMock()
    ):
        proc = Gr00tN1d7Processor.from_pretrained(
            FIXTURE_DIR,
            shortest_image_edge=128,
            crop_fraction=1.0,
    ...
    assert proc.shortest_image_edge == 128 and proc.crop_fraction == 1.0
    assert proc.image_crop_size is None and proc.image_target_size is None
    assert proc.use_percentiles and not proc.clip_outliers
    assert proc.state_action_processor.use_percentiles
    assert not proc.state_action_processor.apply_sincos_state_encoding
    assert not proc.formalize_language
    proc.eval()
    transformed = proc.eval_image_transform(image=np.zeros((720, 1280, 3), dtype=np.uint8))["image"]
    # The new 16:9 camera view must not silently retain a checkpoint's old crop.
    assert transformed.shape == (128, 228, 3)
